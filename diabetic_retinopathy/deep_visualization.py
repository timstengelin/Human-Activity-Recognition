...
def visualize(model, images, labels, step, run_paths):
    ...
    viz_dirs = {
        "original": os.path.join(
            run_paths['path_deep_visualization'], 'original'),
        "grad_cam": os.path.join(
            run_paths['path_deep_visualization'], 'grad_cam'),
        "guided_backprop": os.path.join(
            run_paths['path_deep_visualization'], 'guided_backpropagation'),
        "guided_grad_cam": os.path.join(
            run_paths['path_deep_visualization'], 'guided_grad_cam'),
        "integrated_gradients": os.path.join(
            run_paths['path_deep_visualization'], 'integrated_gradients'),
    }
    ...
    original_path \
        = os.path.join(viz_dirs['original'],
                       f'original_step_{step}_label_{original_label}.png')
    ...
    grad_image, heatmap, pred, grad_path\
        = grad_cam(model, images, step, viz_dirs['grad_cam'])
    cv2.imwrite(grad_path, grad_image)
    logging.info(f'Grad-CAM visualization for step {step} saved at '
                 f'{grad_path}.')

    # Generate and save Guided Backpropagation visualization
    guided_img, guided_path\
        = guided_backpropagation(model, images, step, pred,
                                 viz_dirs['guided_backprop'])
    cv2.imwrite(guided_path, guided_img)
    logging.info(f'Guided Backpropagation visualization for step {step}'
                 f'saved at {guided_path}.')

    # Generate and save Guided Grad-CAM visualization
    guided_cam_img, guided_cam_path\
        = guided_grad_cam(guided_img, heatmap, step, pred,
                          viz_dirs['guided_grad_cam'])
    cv2.imwrite(guided_cam_path, guided_cam_img)
    logging.info(f'Guided Grad-CAM visualization for step {step} saved'
                 f'at {guided_cam_path}.')

    # Generate and save Integrated Gradients visualization
    int_grad_img, int_grad_path\
        = integrated_gradients(model, images, step, pred,
                               viz_dirs['integrated_gradients'])
    cv2.imwrite(int_grad_path, int_grad_img)
    logging.info(f'Integrated Gradients visualization for step {step}'
                 f'saved at {int_grad_path}.')
...
def grad_cam(model, images, step, grad_cam_path):
    ...
    Returns:
        tuple: Grad-CAM visualization, CAM heatmap,
        predicted class index, and image save path
    '''
    ...
    with tf.GradientTape() as tape:
        conv_output, predictions = grad_cam_model(images,
                                                  training=False)
    ...
    input_image = (tf.image.convert_image_dtype(images[0],
                                                dtype=tf.uint8).numpy())
    input_bgr = cv2.cvtColor(input_image, cv2.COLOR_RGB2BGR)
    grad_cam_result = cv2.addWeighted(input_bgr, 1,
                                      cam_colored, 0.3, 0)

    # Define save path
    output_path = os.path.join(grad_cam_path, f'grad_cam_step_{step}_'
                                              f'prediction_'
...
@gin.configurable
def guided_backpropagation(model, images, step,
                           prediction, guided_backprop_path):
    '''
    ...
    Args:
        ...
        guided_backprop_path (str): Directory for saving the
            visualizations
    ...
    norm_grads = ((gradients - np.mean(gradients)) /
                  (np.std(gradients) + 1e-8))
    norm_grads = np.clip((norm_grads * 0.5 + 0.5) * 255,
                         0, 255).astype(np.uint8)

    # Prepare save path
    output_path = os.path.join(
        guided_backprop_path,
        f'guided_backprop_step_{step}_prediction_{prediction}.png')

    return norm_grads[0], output_path


@gin.configurable
def guided_grad_cam(guided_backprop_image, cam, step,
                    prediction, guided_grad_cam_path):
    '''
    Generate the guided Grad-CAM images

    Parameters:
        guided_backprop_image (numpy array): guided backpropagation
            images
        cam (numpy array): CAM mask images
        step (int): number of corresponding model training steps when
            performing deep visualization
        prediction (int): predicted labels of the corresponding images
        guided_grad_cam_path (string): directory where guided Grad-CAM
            images are saved

    Returns:
        guided_grad_cam_image (numpy array): guided Grad-CAM images
        save_path (string): the path and file names of the guided
            Grad-CAM images
    '''
    ...
    guided_grad_cam_image = np.multiply(guided_backprop_image,
                                        cam_reshaped).astype('uint8')
...
@gin.configurable
def integrated_gradients(model, images, step, prediction,
                         integrated_gradients_path, m_steps=50,
                         batch_size=32):
    '''
    ...
    Returns:
        integrated_gradients_image (numpy array): Integrated gradients
            visualization
    ...
    def interpolate(baseline_img, target_img, alphas):
        ...
        delta = (tf.expand_dims(target_img, axis=0) -
                 tf.expand_dims(baseline_img, axis=0))
    ...
    for start_idx in tf.range(0, len(interpolation_steps), batch_size):
        ...
        gradient_store = gradient_store.scatter(tf.range(start_idx,
                                                         end_idx), gradients)
    ...
    def normalize(data):
        return ((data - tf.reduce_mean(data)) /
                (tf.math.reduce_std(data) + 1e-16))
    ...
    scaled_map = ((attribution_map - attribution_map.min()) /
                  (attribution_map.max() -
                   attribution_map.min()) * 255).astype('uint8')
    ...
    overlay = cv2.addWeighted(cv2.cvtColor(original_img,
                                           cv2.COLOR_RGB2BGR), 0.4,
                              heatmap,
                              0.9, 0)

    file_name = (f"integrated_gradients_step_{step}_"
                 f"prediction_{prediction}.png")
