...
def mobilenet_v2(
        input_shape=(
            256,
            256,
            3), n_classes=2, alpha=1.0, dropout_rate=0.2):
    '''
...
def efficientnet_b0(
        input_shape=(
            256,
            256,
            3), n_classes=2, width_coefficient=1.0, depth_coefficient=1.0,
        dropout_rate=0.2):
    '''
...
@gin.configurable
def efficientnet_b3_pretrained(
        input_shape=(
            256,
            256,
            3), n_classes=2, trainable_rate=0.2, dropout_rate=0.2):
    '''
    ...
    Args:
        input_shape (tuple):
            Shape of the input tensor (height, width, channels)
        n_classes (int): Number of output classes
        trainable_rate (float): proportion of trainable parameters in the
    ...
    preprocessed_inputs = tf.keras.applications.efficientnet.preprocess_input(
        inputs)
    ...
    last_nontrainable_layer = len(
        base_model.layers) - int(len(base_model.layers) * trainable_rate)
...
def densenet201_pretrained(
    ...
    # Preprocess input data
    preprocessed_inputs = tf.keras.applications.densenet.preprocess_input(
        inputs)
...
def mobilenet_v2_AND_efficientnet_b0_AND_efficientnet_b3_AND_densenet201(
    ...
    models = [
        mobilenet_v2(
            input_shape=(
                256, 256, 3), n_classes=2), efficientnet_b0(
            input_shape=(
                256, 256, 3), n_classes=2), efficientnet_b3_pretrained(
            input_shape=(
                256, 256, 3), n_classes=2), densenet201_pretrained(
            input_shape=(
                256, 256, 3), n_classes=2)]
